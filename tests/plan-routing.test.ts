...
describe("global parent", () => {
  it("resolves parent user from position parent_id", () => {
    const positions: NetworkPositionRow[] = [
      { id: "pos_c", user_id: "company", plan_id: "PLAN_100", parent_id: null, position: null, depth: 0, cycle: 0 },
      { id: "pos_x", user_id: "X", plan_id: "PLAN_100", parent_id: "pos_c", position: "LEFT", depth: 1, cycle: 0 },
    ];
    expect(globalParentUserId(positions, "X")).toBe("company");
  });
  it("returns null when sponsor is root (Pay must wait)", () => {
    const positions: NetworkPositionRow[] = [
      { id: "pos_x", user_id: "X", plan_id: "PLAN_100", parent_id: null, position: null, depth: 0, cycle: 0 },
    ];
